...
/*
		 ...
		 * Generate Mixed-Integer Linear Programming (MILP) in Related-key setting to obtain security bounds of the minimum number of active S-boxes
		 it apply for both key schedule and state of AES 128-bits
		 * - Original AES Rijndael 128-bit
		 * - key size = 128
		 * - number of rounds = 10
...
public static class minimize
{
		  ...
		  public static int ROUNDS = 10; // number of rounds
		  public static int keyBits = 128; // number of key bits
	public static readonly int blockBits = 128; // number of block bits

	public static readonly int MAXKC = 4; // maximum key size in bits, divided by 32
	public static readonly int MAXBC = 4; // maximum block size in bits, divided by 32
	public static readonly int MAXROUNDS = 10; // maximum number of rounds for AES
	...
	//ORIGINAL LINE: #define SC ((BC - 4) >> 1)
	...
	public static void KeySchedule(int[,] k, int keyBits, int blockBits, int[,,] W)
	{
		  ...
		  int j;
		  int t = 0;
		  int[,] tk = new int[4, MAXKC]; // word8 tk[4][MAXKC];
		...
		for (i = 0 ; i < 4 ; i++)
		{
			W[t / BC, i, t % BC] = tk[i, j]; // for(i = 0; i < 4; i++) W[t / BC][i][t % BC] = tk[i][j];
		}
		}
		 ...
		 W[t / BC, i, t % BC] = tk[i, j] = Xor(tk[i, j], S(tk[(i + 1) % 4, KC - 1])); //  tk[i][0] ^= S[tk[(i+1) %4] [KC-1]];
				}
				}
			 ...
			 if (KC == 4)
			 {
				...
				for (i = 0; i < 4; i++)
				{
					W[t / BC, i, t % BC] = tk[i, j] = Xor(tk[i, j], tk[i, j - 1]); //     for (i =0 ; i < 4 ; i++) tk [i][j] ^= tk[i][j-1];
				}
				}
			 }
			...
			for (j = KC / 2; (j < KC / 2 + 1) && (t < (ROUNDS + 1) * BC); j++, t++) //    for (i =0 ; i <4 ; i++) tk[i][4] ^= S[tk[i][3]];
			{

																								 //  for(j = 5 ; j < KC ; i++)
		 for (i = 0; i < 4; i++)
		 {
			 W[t / BC, i, t % BC] = tk[i, j] = Xor(tk[i, j], S(tk[i, j - 1])); // for( i = 0 ; i < 4; i++) tk[i][j] ^= tk[i][j-1];
		 }
			}
		...
		//	  copy values into round key array 
			  ...
			  for (i = 0; i < 4; i++)
			  {
				  W[t / BC, i, t % BC] = tk[i, j] = Xor(tk[i, j], tk[i, j - 1]); // for (i =0 ; i < 4 ; i++) W[t/ BC] [i][t % BC] ^=tk[i][j];
			  }
				}
	...
	//int main(int argc, const char * argv[]) 
		static int Main()
		{
	  int[,] a = new int[4, MAXBC]; // the bytes of the AES state
	  int[,] k = new int[4, MAXKC]; // the bytes of the AES key
	  int[,,] rk = new int[MAXROUNDS + 1, 4, MAXBC]; // AES round keys
