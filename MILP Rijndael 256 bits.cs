...
/*
		 ...
		 * Generate Mixed-Integer Linear Programming (MILP) in Related-key setting to obtain security bounds of the minimum number of active S-boxes
		 it apply for both key schedule and state of AES 256-bits
		 * - Original AES Rijndael 256-bit
		 * - key size = 128
		 * - number of rounds = 14
...
public static class minimize
{
		  ...
		  public static int ROUNDS = 14; // number of rounds
		  public static int keyBits = 256; // number of key bits
	public static readonly int blockBits = 256; // number of block bits

	public static readonly int MAXKC = 8; // maximum key size in bits, divided by 32
	public static readonly int MAXBC = 8; // maximum block size in bits, divided by 32
	public static readonly int MAXROUNDS = 14; // maximum number of rounds for AES
		  ...
		  public static int KC = 8;
		  public static int BC = 8;
	...
	//ORIGINAL LINE: #define SC ((BC - 8) >> 1)
	...
	public static void KeySchedule(int[,] k, int keyBits, int blockBits, int[,,] W)
	{
		  ...
		  int j;
		  int t = 0;
		  int[,] tk = new int[8, MAXKC]; // word8 tk[8][MAXKC];
		...
		for (i = 0 ; i < 8 ; i++)
		{
			W[t / BC, i, t % BC] = tk[i, j]; // for(i = 0; i < 8; i++) W[t / BC][i][t % BC] = tk[i][j];
		}
		}
		 ...
		 W[t / BC, i, t % BC] = tk[i, j] = Xor(tk[i, j], S(tk[(i + 1) % 8, KC - 1])); //  tk[i][0] ^= S[tk[(i+1) %8] [KC-1]];
				}
				}
			 ...
			 if (KC == 8)
			 {
				...
				for (i = 0; i < 8; i++)
				{
					W[t / BC, i, t % BC] = tk[i, j] = Xor(tk[i, j], tk[i, j - 1]); //     for (i =0 ; i < 8 ; i++) tk [i][j] ^= tk[i][j-1];
				}
				}
			 }
			...
			for (j = KC / 2; (j < KC / 2 + 1) && (t < (ROUNDS + 1) * BC); j++, t++) //    for (i =0 ; i <8 ; i++) tk[i][8] ^= S[tk[i][3]];
			{

																								 //  for(j = 5 ; j < KC ; i++)
		 for (i = 0; i < 8; i++)
		 {
			 W[t / BC, i, t % BC] = tk[i, j] = Xor(tk[i, j], S(tk[i, j - 1])); // for( i = 0 ; i < 8; i++) tk[i][j] ^= tk[i][j-1];
		 }
			}
		...
		//	  copy values into round key array 
			  ...
			  for (i = 0; i < 8; i++)
			  {
				  W[t / BC, i, t % BC] = tk[i, j] = Xor(tk[i, j], tk[i, j - 1]); // for (i =0 ; i < 8 ; i++) W[t/ BC] [i][t % BC] ^=tk[i][j];
			  }
				}
	...
	internal static void ShiftRows(int[,] a, int d, int BC)
	{
		...
		for (i = 1; i < 8; i++)
		{
			for (j = 0; j < BC; j++)
			{
				tmp[j] = a[i, (j + shifts[((BC - 8) >> 1), i, d]) % BC];
			}
			for (j = 0; j < BC; j++)
			{
				a[i, j] = tmp[j];
	...
	//int main(int argc, const char * argv[]) 
		static int Main()
		{
	  int[,] a = new int[8, MAXBC]; // the bytes of the AES state
	  int[,] k = new int[8, MAXKC]; // the bytes of the AES key
	  int[,,] rk = new int[MAXROUNDS + 1, 8, MAXBC]; // AES round keys
